  ...
  Future<void> _loadColorScheme() async {
    if (widget.playlist.image != null) {
      final colorScheme = await ColorScheme.fromImageProvider(
        provider: MemoryImage(widget.playlist.image!),
      ...
      setState(() {
        _colorScheme = null; // Fallback será tratado no build
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        gradient: _colorScheme != null
            ? LinearGradient(
                begin: Alignment.topCenter,
                end: Alignment.bottomCenter,
                colors: [
                  _colorScheme!.primary.withValues(alpha: 0.9),
                  _colorScheme!.surfaceContainer.withValues(alpha: 0.8),
                ],
              )
            : LinearGradient(
                begin: Alignment.topCenter,
                ...
                colors: [
                  context.theme.colorScheme.background.withValues(alpha: 0.9),
                  Colors.gray[800].withValues(alpha: 0.8),
                ],
              ),
      ),
      child: Column(
              ...
              Container(
                ...
                child: ClipRRect(
                  ...
                  child: widget.playlist.image != null
                      ? Image.memory(widget.playlist.image!, fit: BoxFit.cover)
                      : Image.asset('assets/images/record.jpg', fit: BoxFit.cover),
                ),
              ),
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(widget.playlist.name).h1,
                    Text('Playlist de Boel').h3,
                  ],
                ),
              ),
